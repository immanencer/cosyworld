...
class Skull extends DiscordBot {
    ...
    async on_login() {
        ...
        const role = 'system';
        const content = `You are Skull the silent wolf's memory
    ...
    async on_message(message) {
        ...
        console.log(`🐺 Skull heard a whisper from ${data.author} in ${data.location}`);
        ...
        const respond = await ai.currentService.raw_chat('llama3', [
            ...
            write a short limerick ending either in YES or NO
            IF YOUR RESPONSE INCLUDES "YES" THEN SKULL WILL RESPOND
            `}
        ]);
        ...
        const result = await ai.chatSync({
            ...
            content: `
            ${this.message_cache.join('\n')}
            respond as Skull (${this.soul.personality})
            `
        });
        await ai.chat({
            role: 'assistant',
